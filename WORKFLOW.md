...
20. **Discipline scales with the work.** Same principles bind every project; use only the artifacts that change agent behavior. A spike may need posture + research; a regulated product may need specs, ADRs, hooks, and evals. The installer provides every skill; context decides what applies.
21. **Decide when grounded, ask when judgment.** The engineer is the boss, not the co-pilot. The agent is not being watched line by line; it is expected to bring decisions, not fork points. Grounded decisions land without a question — a canonical happy path with citations, a single-criterion winner among TDG approaches, a well-established industry pattern. A material decision keeps its claim-to-source evidence in a durable, auditable project artifact; chat-only citations are not a record. Design/taste, product tradeoffs, irreversible or high-blast-radius actions, and genuinely close calls escalate with a recommendation plus why the alternatives are weaker.
...
24. **Honor personal language preferences.** When a confirmed machine-local personal voice profile defines `languages.conversation` and `languages.publication`, use the conversation language for direct owner conversation and publication previews, and use the publication language as the default for outward text. An explicit language request overrides the matching preference for that request only. A profile without the pair keeps normal contextual inference. Language adaptation never promotes private owner-agent deliberation into outward content.
...
The six layers scale with the work (TL;DR #20 — Discipline scales). A 200-line experiment may legitimately use only Layers 1, 2, and 6 — adding Layers 3, 4, and 5 before they change agent behavior is ceremony. Domain (Layer 2) can still earn its keep early because vocabulary drift starts on day one. A durable, shared, or regulated product often needs all six. The kit installs every skill; the request and repository determine which layers are relevant.
...
**Persist the audit path.** Before a material grounded decision directs repository work, record its conclusion in a versioned project artifact. The record maps each load-bearing claim to a source, includes the source's date and access method, carries the Axis-1 strength and Axis-2 confidence, and names the limitation or evidence that would reverse the decision. The record and the decision cross-reference each other when they are separate artifacts. Structural validation can prove the map is complete; it cannot prove that an external source says what the record claims, so review reopens the cited sources. A chat transcript is not a substitute: it expires, cannot be reliably linked from the changed work, and does not give a later auditor one stable place to begin.
