...
* **Shared-write guard ([ADR-0049](doc/adr/0049-installer-shared-write-target-detection.md)):** the root doc is often a tracked, team-shared file, so a non-interactive run refuses to write its managed section when `trackedState` reports `tracked` — it skips, states why on stderr, and continues; `--force-root-doc` overrides. Interactive prompts name the sharing risk and default to declining. Separately, the installer offers (interactive) to add its freshly-installed, still-untracked files to `.git/info/exclude` by filename — never by directory, since a mixed-ownership `.claude/agents/` can hold a team-tracked file beside the kit's reviewers — and notes when a user-level install already covers the skills.
