...
import { defaultGateCommand, resolveSpawn } from '../scripts/hook-npm-test.js';
...
function mkWiredRepo() {
    ...
    `${JSON.stringify(
      {
        ...
        version: '0.0.1',
        type: 'module',
        scripts: { test: 'node -e ""', verify: 'node -e ""' },
      },
      null,
      2
    )}\n`
  ...
  execFileSync(LEFTHOOK.command, ['install'], {
    cwd: work,
    shell: LEFTHOOK.shell,
  });
  return { dir, work, git };
}

test('regression: task-0033 the verification runner strips the git hook environment', () => {
...
test('wiring: lefthook.yml routes the pre-push test gate through the env-stripping runner', () => {
  ...
  assert.match(
    ...
    'pre-push verification must run via scripts/hook-npm-test.js — a bare command re-opens the worktree env leak'
  );
});

test('wiring: local and CI quality gates share the ADR-0078 verification contract', () => {
  assert.deepEqual(defaultGateCommand(), ['npm', 'run', 'verify']);
  const workflow = readFileSync(join(KIT_ROOT, '.github', 'workflows', 'test.yml'), 'utf8');
...
test(
  'wiring: an empty-diff push updating main is still blocked by branch-guard',
  { skip: !existsSync(LEFTHOOK_SHIM) && 'lefthook binary not installed' },
  () => {
...
test(
  'wiring: a feature-branch push passes the installed pre-push chain',
  { skip: !existsSync(LEFTHOOK_SHIM) && 'lefthook binary not installed' },
  () => {
    const { dir, git } = mkWiredRepo();
    try {
