...
- [x] Portability bar: scripts must run on macOS + Linux + Windows contributors' shells (the CRLF/`core.symlinks` gotchas apply); decide bash-only-with-guard vs Node scripts (`node:` ESM, matching house style).
