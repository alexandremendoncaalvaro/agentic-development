...
- [x] Four-phase body' ' scope intake → stage-split → draft message → sign + write. Conventional Commits format. `Signed-off-by` from `git config user.name` / `user.email`. No `Co-Authored-By` trailer.
