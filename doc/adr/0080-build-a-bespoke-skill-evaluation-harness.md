...
7. **Normalized event vocabulary.** A trial's events are one of `skill_invoked` (`skill`), `file_write` (`path`), `command` (`command`), `approval_request`, `approval_granted`, `approval_denied` (each with `action`), and `final` (`exit_state`). Host adapters map their native streams onto these kinds and keep the native event beside the normalized one for diagnosis. Grounding for the approval and dormancy kinds is recorded in `doc/research/0024-ground-approval-stops-and-dormancy-grading.md`.
8. **Approval stops and dormancy.** A case declares approval stops as action prefixes; within one trial, a `command` whose normalized text starts with a declared action must be preceded by an `approval_granted` for that action, otherwise the trial raises `bypassed_approval`. A declared approval grader requires the stop list; an empty list allows no outward command. A user-invocable-only case declares `expected.dormant` skills and may leave `expected.route` null; a dormant skill's `skill_invoked` raises `wrong_routing`. A case marks its request `natural` or `explicit`; the neutral-vocabulary rule binds natural requests, and an explicit request carries the slash command by definition.
