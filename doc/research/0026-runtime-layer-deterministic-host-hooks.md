...
## Addendum 2026-09-21: the live lane landed the same day

Later on the study date, `feat/live-evaluation-lane` merged into `main` as pull request 145 (merge commit `f879658e`, 15:01 local), carrying ADR-0082 as accepted, Tasks 0081 and 0082, and `eval/lib/live.mjs`. Every "unmerged" qualifier above describes the branch as it stood when the study was written; the numbering prerequisite in Derived decision is moot (ADR-0083 and Task 0083 were free on `main` at merge), and the live lane the third condition depends on is now on the default branch. The Codex leg remains blocked by the operator's Codex CLI, per GROUND-0025's addendum.
