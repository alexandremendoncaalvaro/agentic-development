...
- No PR exists yet → `agentic-pr` (ADR-0024) first.
- Commits are not on the branch yet → `agentic-commit` (ADR-0023) first.
...
- Else `gh pr view --json number,headRefName,baseRefName` against the current branch. If that fails, surface: "No PR found for branch `<name>`. Open one with `agentic-pr` first."
...
- If the merge surfaced a recurring drift: `agentic-audit` for systemic gaps, or update `WORKFLOW.md` §10 / §11.
- If the merge closed a task: confirm the task file's Status is `done` and the Notes log captures the merge commit URL.
- If the merge shipped a binding decision worth recording: `agentic-adr` (three-criteria rule).
