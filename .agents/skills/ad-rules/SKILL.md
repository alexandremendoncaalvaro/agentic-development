...
<instructions>
Step 0 — confirm regime. Run when the user asks to load, see, list, or refresh the rules in force, asks what governs the current work, says "The Rules" or an equivalent rules correction, or starts in a repo whose conventions they have not read.
...
Step 4 — report in inventory mode. When the user asked which rules apply or asked to list their topics, render this inventory and stop:
...
Step 5 — apply correction mode. For "The Rules", `ad-rules`, or an equivalent rules correction, do not dump the inventory unless the user also asked for it.
