...
Implements ADR-0022 (`doc/adr/0022-agentic-grill-skill.md`) — the upstream-of-research phase. Process scaffold for sharpening fuzzy asks before any code, research, or spec work begins. Sits upstream of `agentic-ground`; routes to it (and the other implementation-phase skills) when the question is sharp enough to act on.

No primary file output. Side-effects land in `CONTEXT.md` (via `agentic-domain`) and ADRs (via `agentic-adr`) — both lazy, both belonging to other skills.
...
Term resolved → `agentic-domain`. Route to `agentic-domain` with the canonical noun, the aliases to avoid, and the one-sentence definition. The skill writes to `CONTEXT.md`.

Decision crystallized → maybe `agentic-adr`. Offer an ADR only when **all three** are true (per ADR-0022 §5):
