...
- The bug is a clean ground-truth-pair regression (test was passing, output unchanged, now failing) → `agentic-tdg` (WORKFLOW §9).
- The technique itself is uncertain across multiple plausible approaches → `agentic-spike` (WORKFLOW §14).
- The spec or expected behavior is unclear → `agentic-grill` (ADR-0022).
...
- After fix + regression test land: `/agentic-review main..HEAD` (or current scope) before merge — WORKFLOW §10.
- If Phase 5 found no correct seam for the regression test and the profile is `team` / `mature`: `/agentic-deepen` to surface the deepening opportunity that would create one. On `poc` / `solo`, capture the gap in the commit message and the task Notes — `agentic-deepen` is not installed at those profiles.
...
- If the bug turned out to be a vocabulary drift: `/agentic-domain` to update `CONTEXT.md` and `/agentic-audit` to scan for further drift.
