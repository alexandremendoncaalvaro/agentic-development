...
Step 6 — write the handoff. The handoff's file shape lives in [references/handoff-template.md](references/handoff-template.md) — fill each `<placeholder>` from the state you collected; omit any section that does not apply, except `Roadmap` and `Asks that never landed`, which are never omitted (an empty sweep is written as "none").
