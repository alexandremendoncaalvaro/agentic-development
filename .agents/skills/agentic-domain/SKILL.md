---
name: agentic-domain
description: Lazily create or update `CONTEXT.md` (Layer 2 — ubiquitous language per Evans 2003) at the repo root, or `CONTEXT-MAP.md` plus per-context `CONTEXT.md` for multi-context repos. Captures canonical project-specific nouns, the aliases to avoid, the relationships between them, and resolved ambiguities. Triggers on "domain term", "ubiquitous language", "glossary", "what should we call", "naming drift", "canonical noun", "_Avoid_", "/agentic-domain", or whenever a grilling/spec/architecture session resolves a vocabulary question. Lazy by design — file only exists when there is something to write.
---
...
- `agentic-grill` resolves a term during interview → routes here.
- `agentic-spec` introduces a new noun while drafting a spec → routes here.
- `agentic-architecture` names a domain-bound concept → routes here.
- `agentic-audit` detects code/glossary drift → routes here.
- Direct `/agentic-domain` invocation when the user wants to add a term explicitly.
...
Step 2 — locate the insertion point. `CONTEXT.md` has three canonical sections (per ADR-0019 §2):
...
_Maintained by `/agentic-domain` per ADR-0019._
...
Cross-references are *suggestions*, not file edits — the skill does not modify specs or ADRs to chase drift. That work belongs to `agentic-audit`.

Step 5 — `AGENTS.md` pointer (one-time). When `CONTEXT.md` is created for the first time, the artifact stack reference in `AGENTS.md` should mention it. Per ADR-0019 §8, the `agentic-bootstrap` skill inserts the pointer on its next run. This skill does not edit `AGENTS.md` directly; it announces the side-effect:

> "Created `CONTEXT.md`. Next `agentic-bootstrap` run will insert the Layer 2 pointer in `AGENTS.md`."
</instructions>
...
- After capture: route back to the calling skill (`agentic-grill`, `agentic-spec`, `agentic-architecture`) to resume the original turn.
- If the resolution surfaced a binding decision (hard to reverse, surprising without context, real trade-off): `/agentic-adr` to record it.
- If the term is one of several pending resolutions: stay in `/agentic-grill` to walk the next branch of the design tree.
- For periodic drift sweeps: `/agentic-audit` checks `CONTEXT.md` against current code.
