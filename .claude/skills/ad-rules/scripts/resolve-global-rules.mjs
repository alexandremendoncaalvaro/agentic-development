#!/usr/bin/env node
/**
 * Resolve global rules for the Claude Code host (ADR-0057, P2.8).
 * Run from any consumer repository:
 *
 *   node <skill-base-dir>/scripts/resolve-global-rules.mjs
 *
 * The Claude Code location wins, then the remaining canonical paths are
 * reported as fallbacks. This script is intentionally host-divergent from the
 * Codex copy: each host must prefer its own instruction file. It probes that
 * a source is readable but emits no content and performs no write; the
 * SKILL.md body extracts topics and makes all judgment calls.
 */
