...
- A one-line anchor echo: `Anchors: <file>=<sha256>, target=<SHA>`. Recompute each rule file listed under the handoff's EXPECTED ANCHORS yourself after reading it (`shasum -a 256 <file>` via Bash, resolving the file from its `(file: <absolute path>)` annotation) and state the tree SHA you actually audited — computed and observed values, never copied from the handoff. When the handoff expects no file anchors (binding-doc group), echo `Anchors: target=<SHA>` alone. A computed value that disagrees with the handoff's expectation is itself a **critical** finding (stale or divergent audit context) — report it; do not proceed past it silently. If the handoff's inline GROUP RULES text disagrees with the anchored file's on-disk content, report that divergence too — it is the same stale-context class. Without this line the calling agent treats your verdicts as UNVERIFIED.
