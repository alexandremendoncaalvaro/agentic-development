...
* **Every skill is model-invocable and gates its own effects (ADR-0085, amending ADR-0073).** A skill runs every outward effect (push, pull request, merge, tag, release, publish, post) and every irreversible one (files outside the repository, machine stores, deletions git cannot restore) only after an approval step stated in its body; with that step written it carries no `disable-model-invocation` on Claude Code and `policy.allow_implicit_invocation: true` in the Codex `agents/openai.yaml`. The host block is reserved for a skill that cannot gate its effect, and the kit ships none; `test/skills.test.js` fails when either flag appears outside its empty exception set. Every `description:` is capped at 350 chars, use case and triggers first, and at the 1,024-char spec maximum; no whole-collection total is enforced, because the host's built-in skills share the listing and the kit overflows a 200k-window budget with or without the block (RESEARCH-0030). A dot-directory under `src/skills/<host>/` is never a skill — `bundledSkills` and the test enumerator skip it (a plugin once dropped `.slim/` there and 14 tests went red).
...
| `ad-hooks` | `/ad-hooks` | Scaffold deterministic quality gates per WORKFLOW §11 — pre-commit + pre-push, runner detected from stack signals — plus a session-lifecycle tier (a Stop handoff nudge, a UserPromptSubmit workflow checkpoint, and a dual-host PostToolUse artifact-validator gate). |
