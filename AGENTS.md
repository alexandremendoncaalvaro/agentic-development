...
| `ad-review` | `/ad-review` | Two-axis fresh-context code review per WORKFLOW §10. Standards (binding docs: AGENTS.md / ARCHITECTURE.md / GUIDELINES.md / CONTEXT.md / accepted ADRs) and Spec (originating task / spec / PRD) run as parallel sub-agents — Claude Code spawns two `Task` calls, Codex runs two sequential `/clear` handoffs. Reports side-by-side; no cross-axis re-ranking. Adversarial — no "approve" verdict. |
