...
* **Skill set:** twenty universal at `team` / `mature` profile — `ad-bootstrap`, `ad-philosophy`, `ad-architecture`, `ad-adr`, `ad-prd`, `ad-spec`, `ad-task`, `ad-audit`, `ad-review`, `ad-ground`, `ad-next`, `ad-spike`, `ad-tdg`, `ad-domain`, `ad-grill`, `ad-deepen`, `ad-diagnose`, `ad-commit`, `ad-pr`, `ad-merge`. `solo`-and-up only: `ad-prd`, `ad-commit`, `ad-pr`, `ad-merge`. `team`-and-up only: `ad-deepen`. Plus four conditional: `ad-design` (auto if `detectFeatures().frontend`), `ad-subagent` (auto for Claude Code), `ad-skill` (opt-in), `ad-hooks` (opt-in / recommended at `mature`). Profile-specific install sets: `poc` = 9, `solo` = 17, `team` / `mature` = 20. Source under `src/lib/profiles.js`.
