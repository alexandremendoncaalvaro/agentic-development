...
| `ad-review` | `/ad-review` | Two-axis code review per WORKFLOW §10. Standards (binding docs: AGENTS.md / ARCHITECTURE.md / GUIDELINES.md / CONTEXT.md / accepted ADRs) and Spec (originating task / spec / PRD). Claude Code spawns two parallel `Task` sub-agents with fresh context. Codex runs a single-session pass with axis-separated output (Codex skills cannot programmatically spawn sub-agents; documented user-initiated subagent escalation is offered for §10 ideal). Reports side-by-side; no cross-axis re-ranking. Adversarial — no "approve" verdict. |
...
| `ad-handoff` | `/ad-handoff` | Compact the current session into a handoff document in `${TMPDIR:-/tmp}/agentic-handoffs/<ISO>-<slug>.md`. Captures live state (branch, open artifacts, unresolved decisions, in-progress diff, recent errors), references PRD / spec / task / ADR by path (no duplication), suggests next skills, redacts secrets. Ephemeral — never commits to the repo. |
