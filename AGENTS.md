...
| `ad-rules` | `/ad-rules` | Resolve the rule-set in force (repo binding docs + machine store + project rules per ADR-0035/0043) and surface it as a topic list. Discovers a practitioner global rules file at a non-standard path and prints the one-line fix to make it resolve automatically. Read-only; audits nothing. |
