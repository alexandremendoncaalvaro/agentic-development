...
* **Skill set:** twenty-one universal at `team` / `mature` profile — `ad-bootstrap`, `ad-philosophy`, `ad-architecture`, `ad-adr`, `ad-prd`, `ad-spec`, `ad-task`, `ad-audit`, `ad-review`, `ad-ground`, `ad-next`, `ad-spike`, `ad-tdg`, `ad-tdd`, `ad-domain`, `ad-grill`, `ad-deepen`, `ad-diagnose`, `ad-commit`, `ad-pr`, `ad-merge`. `solo`-and-up only: `ad-prd`, `ad-commit`, `ad-pr`, `ad-merge`. `team`-and-up only: `ad-deepen`. Plus four conditional: `ad-design` (auto if `detectFeatures().frontend`), `ad-subagent` (auto for Claude Code), `ad-skill` (opt-in), `ad-hooks` (opt-in / recommended at `mature`). Profile-specific install sets: `poc` = 10, `solo` = 18, `team` / `mature` = 21. Source under `src/lib/profiles.js`.
