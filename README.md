...
| `ad-review` | workflow-operational | universal | Two-axis fresh-context code review per WORKFLOW §10. Standards axis (binding docs — AGENTS.md / ARCHITECTURE.md / GUIDELINES.md / CONTEXT.md / accepted ADRs) and Spec axis (originating task / spec / PRD) run as parallel sub-agents (Claude Code) or two sequential `/clear` handoffs (Codex); reports side-by-side, no cross-axis re-ranking, no "approve" verdict | `/ad-review <range>` |
