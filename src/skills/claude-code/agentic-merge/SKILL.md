---
name: agentic-merge
description: Evaluate and merge a GitHub pull request per ADR-0025. Four phases — preflight (`gh` auth + PR resolution), evaluate (CI / fresh-context review / linked task / unresolved comments / mergeability), decision (CI green = hard gate; others = warnings yielding to user), merge via `gh pr merge` with auto-detected mode (squash / rebase / merge) and `--delete-branch`. Helper posture per ADR-0025 §3 — surfaces warnings, does not block on the senior engineer's judgment. Triggers on "merge this PR", "evaluate the PR", "is it mergeable", "gh pr merge", "/agentic-merge".
...
- No PR exists yet → `/agentic-pr` ([ADR-0024](../../doc/adr/0024-agentic-pr-skill.md)) first.
- Commits are not on the branch yet → `/agentic-commit` ([ADR-0023](../../doc/adr/0023-agentic-commit-skill.md)) first.
...
If absent or not authed, surface the install / `gh auth login` hint and stop (same soft-fail rule as `agentic-pr`).

Resolve the target PR:

- If the user passed a PR number / URL, use it.
- Else `gh pr view --json number,headRefName,baseRefName` against the current branch. If that fails (no PR yet for the branch), surface: "No PR found for branch `<name>`. Open one with `/agentic-pr` first."
...
- If the merge surfaced a recurring drift (no fresh-context review on multiple PRs, no linked task on several merges): `/agentic-audit` to scan for systemic gaps, or update `WORKFLOW.md` §10 / §11 expectations.
- If the merge closed a task: confirm the task file's `Status:` is `done` and the Notes log captures the merge commit URL.
- If the merge shipped a binding decision worth recording: `/agentic-adr` (three-criteria rule per [ADR-0022](../../doc/adr/0022-agentic-grill-skill.md) §5 — hard to reverse, surprising without context, real trade-off).
