...
_Maintained by `/agentic-domain` per [ADR-0019](doc/adr/0019-domain-language-layer.md)._
...
These cross-references are *suggestions*, not file edits — the skill does not modify specs or ADRs to chase drift. That work is `agentic-audit`'s.

## Step 5 — `AGENTS.md` pointer (one-time)

When `CONTEXT.md` is created for the first time, the artifact stack reference in `AGENTS.md` should mention it. Per ADR-0019 §8, the `agentic-bootstrap` skill inserts the pointer on its next run. This skill does not edit `AGENTS.md` directly; it announces the side-effect:
