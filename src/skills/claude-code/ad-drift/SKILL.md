...
The mechanical checks — artifact numbering, `Status:` validity, supersession-target existence, amendment-pair matching, emoji in narrative docs, and checkbox UI in definition docs / specs — are a bundled script (ADR-0057), not prose to re-derive by hand. Run it from the repo root and read its JSON:

```bash
node .claude/skills/ad-drift/scripts/drift-scan.mjs
```
...
* Reciprocity — the scan does not compute this; read it. Every task under `doc/tasks/NNNN-*.md` whose `Spec ref` field is non-empty points to a spec that exists. And every spec with `Status: accepted` or `shipped` has at least one entry in its `Related → Tasks` list (an accepted spec with no implementing task is a smell).
* No checkbox UI — per ADR-0030 §1, Spec is decision-record (not tracking). Functional Requirements / Non-functional Requirements / Success Criteria sections must use plain bullets, not `- [ ]` checkboxes; implementation tracking lives in per-Spec tasks. `checkbox` (its `doc/specs/` entries) pre-computes these.
* Status / task aggregate alignment — the scan does not compute this; read it. When every task that references a spec is `done`, the spec's `Status` should be `shipped`. A spec with all tasks done but `Status: accepted` is drift between work-unit completion and feature-level claim.
...
* **Definition-layer tracking UI** (Rule #9) — `checkbox` lists checkbox UI (`{path, line}`) in `AGENTS.md`, `WORKFLOW.md`, `ARCHITECTURE.md`, `GUIDELINES.md`, `CONTEXT.md`, `doc/product/*.md`, and specs, with fenced code-block examples (e.g., PR-body shape) already excluded. Definition documents must not carry per-item checkbox UI.
