...
- The branch has uncommitted or unsplit work → `/agentic-commit` first ([ADR-0023](../../doc/adr/0023-agentic-commit-skill.md)).
- The user wants to merge an already-open PR → `/agentic-merge` ([ADR-0025](../../doc/adr/0025-agentic-merge-skill.md)).
...
- When CI is green: `/agentic-merge` ([ADR-0025](../../doc/adr/0025-agentic-merge-skill.md)) to evaluate and merge.
- If a fresh-context review is wanted before merge: `/agentic-review` (WORKFLOW §10) — the review artifact lands in `.agentic/reviews/` and `agentic-merge` checks for it.
- If preflight surfaced "no CI workflow file": `/agentic-hooks` (when installed) covers local gates; remote CI is a separate concern outside this skill.
