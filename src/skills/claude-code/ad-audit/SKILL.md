...
- **Procedure.** Apply the minimal mutation the test should catch → run the CI-exact filter → observe `{build result, effect/asset presence, pass-fail count}` → restore the tree → confirm `git status` clean. Green (the suite stayed green through the mutation) confirms the finding; red refutes it.
- **Serial and isolated.** Run only after the Step 4 / Step 5 parallel reviewers have finished — never concurrent with them; a heavy suite alongside the fan-out is what drove a 15-core machine to load 113. One mutation at a time.
