...
* No `## Active ADRs` section — ARCHITECTURE.md must not duplicate the `doc/adr/` directory index per [ADR-0030](../../doc/adr/0030-single-responsibility-per-document.md) §2.
...
* No checkbox UI — per [ADR-0030](../../doc/adr/0030-single-responsibility-per-document.md) §1, Spec is decision-record (not tracking). Functional Requirements / Non-functional Requirements / Success Criteria sections must use plain bullets, not `- [ ]` checkboxes; implementation tracking lives in per-Spec tasks.
...
### Single-responsibility drift ([ADR-0030](../../doc/adr/0030-single-responsibility-per-document.md))
