...
- **Source confidence.** Name the primary evidence source and observation time.
  If `ad-project-state` reports a partial or unavailable source, retain local
...
- Use `/ad-project-state` when the source selection or an external-source failure
