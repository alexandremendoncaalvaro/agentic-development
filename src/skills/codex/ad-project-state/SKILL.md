...
```bash
node .agents/skills/ad-project-state/scripts/project-state.mjs
```
...
Interpret the packet as follows:
...
Keep composition one-way. Return the packet to the calling state reader.
`ad-next` owns scenario classification and priorities; project-scoped
`ad-roadmap` owns plan selection, reconciliation, progress, and checklists;
standalone `ad-brief` owns the decision-maker presentation. When a specialist
already supplied settled facts, do not recollect or reinterpret them.
