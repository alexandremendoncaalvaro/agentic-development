...
Step 3 — write the file(s). If the target file already exists, show it and overwrite it only after the user approves; a personal-scope file lives outside the repository, so git cannot restore it.
...
- Claude Code: `name`, `description`, `allowed-tools` (and any other field from
  the spec the user asked for). Omit `disable-model-invocation`; add
